				...
				<div class="box-body box-profile">
					<div class="col-md-12 col-xs-12">
						@if(in_array(Auth::user()->UsRol, Permisos::PersInter1) || in_array(Auth::user()->UsRol2, Permisos::PersInter1))
							<a href="/usuariosexternos/{{$User->UsSlug}}/edit" class="btn btn-warning pull-right"><i class="fas fa-edit"></i><b> {{ trans('adminlte_lang::message.edit') }}</b></a>
						@endif
						...
						@if ($User->FK_UserPers === null)
							@if($User->DeleteUser === 0)
								...
								@if(in_array(Auth::user()->UsRol, Permisos::PersInter1) || in_array(Auth::user()->UsRol2, Permisos::PersInter1))
									<form action='/usuariosexternos/{{$User->UsSlug}}' method='POST'  class="col-12 pull-left">
										@method('DELETE')
										@csrf
							...
							@else
								@if (in_array(Auth::user()->UsRol, Permisos::PersInter1) || in_array(Auth::user()->UsRol2, Permisos::PersInter1))
									<form action='/usuariosexternos/{{$User->UsSlug}}' method='POST' class="pull-left">
										@method('DELETE')
										@csrf
					...
					<div class="text-center">
						<a href="/usuariosexternos/{{$User->UsSlug}}/editpassword" class="btn btn-info"><i class="fas fa-key"></i><b> {{ trans('adminlte_lang::message.userchangepassword') }}</b></a>
					</div>
				</div>
			</div>
		</div>

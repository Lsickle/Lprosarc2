			...
			@foreach($opcion->tarifas as $tarifa)
					...
					@if(count($tarifa->rangos) > 0)
						...
						<script type="text/javaScript">
							...
							<div class="col-md-3" id="rango{{$contadorphp}}{{$last}}">
								...
								<input {{in_array(Auth::user()->UsRol, Permisos::ComercialYJefeComercial)||in_array(Auth::user()->UsRol2, Permisos::ComercialYJefeComercial) ? '' : 'disabled' }} id="rangopriceinput{{$contadorphp}}{{$last}}" name="Opcion[{{$contadorphp}}][TarifaPrecio][]" type="number" class="form-control  addon-inline" placeholder="Precio" min="0" value="{{$rango->TarifaPrecio}}">
			    	...
			    	@else
		    			...
		    			<div class="col-md-3" id="rango{{$contadorphp}}{{$last}}">
		    				...
		    				<input {{in_array(Auth::user()->UsRol, Permisos::ComercialYJefeComercial)||in_array(Auth::user()->UsRol2, Permisos::ComercialYJefeComercial) ? '' : 'disabled' }} id="rangopriceinput{{$contadorphp}}{{$last}}" name="Opcion[{{$contadorphp}}][TarifaPrecio][]" type="number" class="form-control" placeholder="Precio" min="0">
